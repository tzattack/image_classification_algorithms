...
class TFRecord:
    ...
    def image_data_write_jpeg(self,image_data,filepath,label=0):
...
def write_label_file(train_data_dir, labels):
    labels = dict(zip(range(len(labels)), labels))
    labels_filename = os.path.join(train_data_dir, 'labels.txt')
...
if __name__== '__main__':
    filename = "./data_input/test.tfrecord"
    tf_data = TFRecord()
    tf_data.image_data_writer_open(filename)
    raw_data_dir ,train_data_dir = './data_raw/test', './data_input'

    for _, dirs, files in os.walk("./data_raw/test"):
        tf_data.image_data_writer_open(filename)
        for file in files:
            if file == '.DS_Store': continue
            try:
                image = Image.open(os.path.join(raw_data_dir, file))
            except:
                print("skip file {}...".format(file))
                continue
            image_arr = np.array(image)
            tf_data.image_data_write_jpeg(image_arr, file, 'test')
